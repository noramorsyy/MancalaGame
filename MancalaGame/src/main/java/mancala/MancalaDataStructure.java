...
public class MancalaDataStructure implements Serializable {
    static private final int PLAYER_ONE = 6;
    static private final int PLAYER_TWO = 13;
    ...
    /**
     * Gets the store of the corresponding player 
     * @param pNum The player number (1 or 2).
     * @return The position of the corresponding store 
     */
    public Store getStore(final int pNum) {
        return (Store) data.get(storePos(pNum));
    }
     ...
     * gets the store of the corresponding player
     * @param playerNum The player number (1 or 2).
     * @return the store of the corresponding player 
     */
    public int getStorePos(final int playerNum) {
        if(playerNum <1 || playerNum > 2){
            throw new RuntimeException("Invalid Player Position");
        }
